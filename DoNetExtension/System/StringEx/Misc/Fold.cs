namespace System
{
    public partial class StringEx
    {
        ...
        public static string FoldToASCII(this string str, int start, int length)
        {
            unsafe
            {
                fixed (char* p = str)
                {
                    var output = new char[length + length + length + length];
                    var outputPos = _foldToASCII(p, start, length, output, 0);
        ...
        /// <remarks>Based on Apache Lucence.Net implementation at https://github.com/apache/lucenenet. </remarks>
        public static string FoldToASCII(this string str)
        {
            ...
            unsafe
            {
                ...
                {
                    var output = new char[length + length + length + length];
                    var outputPos = _foldToASCII(p, 0, length, output, 0);
                    return new string(output, 0, outputPos);
                }
            }
        }

        static unsafe int _foldToASCII(char* input, int inputPos, int length, char[] output, int outputPos)
        {
            var end = inputPos + length;
            for (var pos = inputPos; pos < end; ++pos)
            {
                var c = input[pos];
