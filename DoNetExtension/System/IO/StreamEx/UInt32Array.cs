...
namespace System.IO
{
	public static partial class StreamEx
	{
		...
		/// </param>
		public unsafe static void WriteUInt32s(this Stream stream, UInt32[] array, int startIndex, int count)
		{
			WriteUInt32s(stream, array, startIndex, count, new byte[count * sizeof(UInt32)]);
		}
		...
		public unsafe static void WriteUInt32s(this Stream stream, UInt32[] array, int startIndex, int count, byte[] buffer)
		{
			fixed (byte* ptr = buffer)
			{
                Buffer.BlockCopy(array, 0, buffer, 0, sizeof(UInt32) * count);
			}
		...
		/// <para>!!! Note this number should be no larger than the number of integers from <paramref name="startIndex" /> to the end of the array.</para></param>
		/// <param name="buffer">A byte array used to temporarily store data to write.</param>
		public unsafe static void WriteUInt32s(this Stream stream, IList<UInt32> array, int startIndex, int count)
		{
			WriteUInt32s(stream, array, startIndex, count, new byte[sizeof(UInt32) * count]);
		}
		...
		/// <para>!!! Note this number should be no larger than the number of integers from <paramref name="startIndex" /> to the end of the array.</para></param>
		/// <param name="buffer">A byte array used to temporarily store data to write.</param>
		public unsafe static void WriteUInt32s(this Stream stream, IList<UInt32> list, int startIndex, int count, byte[] buffer)
		{
			fixed (byte* ptr = buffer) 
			{
				UInt32* iptr2 = (UInt32*)ptr;
		...
		public unsafe static void WriteUInt32s(this Stream stream, UInt32[] array, int startIndex = 0)
		{
			var count = array.Length - startIndex;
			WriteUInt32s(stream, array, startIndex, count);
		}
		...
		/// <param name="startIndex">A position in the array where the writing starts.</param>
		/// <param name="buffer">A byte array used to temporarily store data to write.</param>
		public unsafe static void WriteUInt32s(this Stream stream, UInt32[] array, byte[] buffer, int startIndex = 0)
		{
			var count = array.Length - startIndex;
			WriteUInt32s(stream, array, startIndex, count, buffer);
		}
		...
		/// <param name="startIndex">A position in the list where the writing starts.</param>
		public unsafe static void WriteUInt32s(this Stream stream, IList<UInt32> list, int startIndex = 0)
		{
			var count = list.Count - startIndex;
			WriteUInt32s(stream, list, startIndex, count);
		}
		...
		/// <param name="startIndex">A position in the list where the writing starts.</param>
		/// <param name="buffer">A byte array used to temporarily store data to write.</param>
		public unsafe static void WriteUInt32s(this Stream stream, IList<UInt32> list, byte[] buffer, int startIndex = 0)
		{
			var count = list.Count - startIndex;
			WriteUInt32s(stream, list, startIndex, count, buffer);
		}
		...
		public unsafe static UInt32[] ReadUInt32s(this Stream stream, int length)
		{
			var data = new UInt32[length];
			var dataLen = length * sizeof(UInt32);
			byte[] buffer = stream.ReadBytes(dataLen);

			fixed (UInt32* lptr = data)
			{
				var bptr = (byte*)lptr;
		...
		/// <returns>A System.UInt32 integer array read from the stream.</returns>
		public unsafe static UInt32[] ReadUInt32s(this Stream stream, int length, byte[] buffer)
		{
			var data = new UInt32[length];
			var dataLen = length * sizeof(UInt32);
		...
		public static int WriteUInt32Array(this Stream stream, UInt32[] array, bool validityCheck = true)
		{
			var metaLen = 4;
			if (validityCheck)
			{
			...
			stream.WriteUInt32s(array);
			return metaLen + arrLen * sizeof(UInt32);
		}
		...
		/// <param name="buffer">A byte array used to temporarily store data to write.</param>
		/// <returns>The number of bytes actually written to the stream.</returns>
		public static int WriteUInt32Array(this Stream stream, UInt32[] array, byte[] buffer, bool validityCheck = true)
		{
			var metaLen = 4;
			if (validityCheck)
			{
			...
			stream.WriteUInt32s(array, buffer);
			return metaLen + arrLen * sizeof(UInt32);
		}
		...
		/// <summary>
		/// Reads a System.UInt32 integer array from this stream. The method does not require an argument specifying the length of the array; <seealso cref="ReadUInt32s"/>.
		/// </summary>
		/// <param name="stream">The stream to read.</param>
		...
		public static UInt32[] ReadUInt32Array(this Stream stream, bool validityCheck = true)
		{
		...
		/// </returns>
		/// <exception cref="System.IO.InvalidDataException">Raises if data in the stream is corrupted.</exception>
		public static UInt32[] ReadUInt32Array(this Stream stream, byte[] buffer, bool validityCheck = true)
		{
			if (!validityCheck || stream.Check((long)37))
			{
				...
				return stream.ReadUInt32s(len, buffer);
			}
			else throw new InvalidDataException(IOResources.ERR_StreamExtension_DataIrrecognizable);
		}
		...
		/// </returns>
		/// <exception cref="System.IO.InvalidDataException">Raises if data in the stream is corrupted.</exception>
		public static bool ReadUInt32ArrayToBuffer(this Stream stream, byte[] buffer, bool validityCheck = true)
		{
			if (!validityCheck || stream.Check((long)37))
			{
				var len = stream.ReadInt32(buffer);
				...
				stream.Read(buffer, 0, len * sizeof(UInt32));
				return true;
			}
			else throw new InvalidDataException(IOResources.ERR_StreamExtension_DataIrrecognizable);
		}
